@props(['title', 'type' => 'submit', 'class' => '', 'tabindex' => '0', 'noSvg' => false])

<button type="{{ $type }}" tabindex="{{ $tabindex }}" title="{{ $title }}"
        class="relative group cursor-pointer focus:outline-none focus:ring-0 {{ $class }}" {{ $attributes }}>
    {{ $slot }}
